                ...
                children: [
                  ...
                  BlocBuilder<CardBloc, CardState>(
                    buildWhen: (previous, current) {
                      if (previous.typeCardModel.uid !=
                          current.typeCardModel.uid) {
                        _cardNumber.clear();
                        _cardDate.clear();
                        _cardCvv.clear();
                      }
                    ...
                    builder: (_, state) {
                      return Container(
                        padding: const EdgeInsets.only(
                            left: 20.0, top: 5.0, bottom: 5.0, right: 5.0),
                        height: 55,
                      ...
                      Flexible(
                        child: Container(
                          ...
                          child: TextFormField(
                            ...
                            decoration: const InputDecoration(
                                border: InputBorder.none, hintText: 'XX/XX'),
                          ),
                        ),
                      ),
                      ...
                      itemBuilder: (context, i) {
                        return Padding(
                          ...
                          child: GestureDetector(
                            onTap: () {
                              cardBloc.add(SelectTypeColorEvent(
                                  TypeCardColor.typeCardColor[i]));
                            },
                            child: BlocBuilder<CardBloc, CardState>(
                              builder: (_, state) {
                                return Container(
                                    ...
                                    width: 32,
                                    decoration: BoxDecoration(
                                        color: Color(TypeCardColor
                                            .typeCardColor[i].color),
                                        shape: BoxShape.circle),
                                    child: TypeCardColor.typeCardColor[i].uid ==
                                            state.typeCardColor.uid
                ...
                onTap: () {
                  bool validateForm() {
                    ...
                    if (_cardNumber.text.isEmpty)
                      _emptyFields += '• Number is required\n';
                    if (_cardDate.text.isEmpty)
                      _emptyFields += '• Date is required\n';
                    if (_cardCvv.text.isEmpty)
                      _emptyFields += '• CVV is required\n';
                    if (state.typeCardModel.uid == 3 &&
                        _cardNumber.text.trim().length != 17)
                      _emptyFields += '• Amex Card Number\n';
                    if (state.typeCardModel.uid == 8 &&
                        _cardNumber.text.trim().length != 16)
                      _emptyFields += '• Diners Club Number\n';
                    if (_cardNumber.text.trim().isNotEmpty &&
                        state.typeCardModel.uid == 3 &&
                        _cardNumber.text.length != 17)
                      _emptyFields += '• Card Number\n';
                    if (_cardNumber.text.trim().isNotEmpty &&
                        state.typeCardModel.uid == 8 &&
                    ...
                    if (_cardNumber.text.trim().isNotEmpty &&
                        state.typeCardModel.uid == 3 &&
                        _cardCvv.text.trim().length != 4)
                      _emptyFields += '• Card CVV\n';
                    if (_cardNumber.text.trim().isNotEmpty &&
                        state.typeCardModel.uid == 8 &&
