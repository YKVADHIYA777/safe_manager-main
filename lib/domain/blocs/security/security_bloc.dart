...
class SecurityBloc extends Bloc<SecurityEvent, SecurityState> {
  ...
  Future<void> _selectNumberVerifyPin(
      SelectNumberVerifyPinEvent event, Emitter<SecurityState> emit) async {
  ...
  Future<void> _selectNumberNewPin(
      SelectNumberNewPinEvent event, Emitter<SecurityState> emit) async {
    if (state.newNumbers.length <= 5) {
      emit(state.copyWith(
          newNumbers: List.of(state.newNumbers)..add(event.number)));
  ...
  Future<void> _clearLastNumberVerifyPin(
      ClearLastNumberVerifyPinEvent event, Emitter<SecurityState> emit) async {
  ...
  Future<void> _clearLastNumberNewPin(
      ClearLastNumberNewPinEvent event, Emitter<SecurityState> emit) async {
    if (state.newNumbers.isNotEmpty) {
      return emit(state.copyWith(
          newNumbers: List.of(state.newNumbers)..removeAt(event.index - 1)));
    }
  ...
  Future<void> _verifyPin(
      VerifyPinEvent event, Emitter<SecurityState> emit) async {
    ...
    if (isEquals) {
      ...
      return emit(
          state.copyWith(isFailurePassword: true, isSuccessPassword: false));
    }
  }

  Future<void> _saveNewPin(
      SaveNewPinEvent event, Emitter<SecurityState> emit) async {
  ...
  Future<void> _clearAllPin(
      ClearAllPinEvent event, Emitter<SecurityState> emit) async {
    return emit(state.copyWith(
